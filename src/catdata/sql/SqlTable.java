...
public class SqlTable {
	...
	public void validate() {
		if (name == null) {
			throw new RuntimeException("anomaly");
		}
		...
		if (!columns.containsAll(pk)) {
			throw new RuntimeException("Missing PK " + Util.sep(pk, ",") + " in " + Util.sep(columns, ","));
		}
		if (columns.size() != new THashSet<>(columns).size()) {
			throw new RuntimeException("Duplicate columns: " + Util.sep(columns, ","));
		}
	}
