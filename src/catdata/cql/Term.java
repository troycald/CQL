...
public final class Term<Ty, En, Sym, Fk, Att, Gen, Sk> implements KBExp<Head<Ty, En, Sym, Fk, Att, Gen, Sk>, String> {
	...
	public <X> X visit(Function<String, X> varf, BiFunction<Object, Ty, X> tyf, BiFunction<Sym, List<X>, X> symf,
	...
	// @SuppressWarnings("hiding")
	// private <Ty, En, Sym, Fk, Att, Gen, Sk> Term<Ty, En, Sym, Fk, Att, Gen, Sk>
	// asVar() {
	// if (var != null) {
	// return Var(var);
	// }
	...
	public boolean hasTypeType(Map<String, Chc<Ty, En>> map) {
		if (var != null) {
	...
	public <Ty, En> boolean monoidal(Map<String, Chc<Ty, En>> ctx) {
		if (obj() != null) {
			return true;
		} else if (sk() != null) {
			return true;
	...
	public Chc<Ty, En> type(Map<String, Ty> ctxt, Map<String, En> ctxe, Set<Ty> tys, Map<Sym, Pair<List<Ty>, Ty>> syms,
		...
		if (var != null) {
			...
			if (ctxe.containsKey(var)) {
				...
				throw new RuntimeException("In " + this + ", " + var + " is not a variable in type context [" + ctxt
						+ "] and entity context [" + ctxe + "]");
			}
		} else if (obj() != null) {
		...
		if (ret == null || (ret.left && ret.l == null) || (!ret.left && ret.r == null)) {
			...
			throw new RuntimeException("In " + this + "," + " return type is " + ret.l
					+ " which is not a type.  Available: " + Util.sep(tys, ", "));
		} else if (!ret.left && !ens.contains(ret.r)) {
			throw new RuntimeException("In " + this + "," + " return type is " + ret.r
					+ " which is not a entity.    Available: " + Util.sep(ens, ", "));
		}
		return ret;
	}
	...
	private synchronized static <Ty, En, Sym, Fk, Att, Gen, Sk> Term<Ty, En, Sym, Fk, Att, Gen, Sk> mkTerm(String var,
	...
	private Term(String var, Sym sym, Fk fk, Att att, Gen gen, Sk sk, List<Term<Ty, En, Sym, Fk, Att, Gen, Sk>> args,
			Term<Ty, En, Sym, Fk, Att, Gen, Sk> arg, Object obj, Ty ty) {
		this.var = var;
		this.args = args;
		this.arg = arg;
	...
	public String toStringSql(Schema sch, String tick, boolean convert) {
		if (var != null) {
			...
			if (args.isEmpty()) {
				var ret = sym().toString();
				return ret;
			}
			if (sym().toString().toLowerCase().startsWith("null_") & args().size() == 2) {
				return "null";
			}
			if (sym().toString().toLowerCase().startsWith("eq_") & args().size() == 2) {
				var ret = args.get(0).toStringSql(sch, tick, convert) + " = " + args.get(1).toStringSql(sch, tick, convert);
				return ret;
			}
			if (sym().toString().toLowerCase().startsWith("and") & args().size() == 2) {
				var ret = args.get(0).toStringSql(sch, tick, convert) + " and " + args.get(1).toStringSql(sch, tick, convert);
				return ret;
			}
	...
	static int sqlVar = 0;
	static Map<String, String> sqlVars = new THashMap<>();

	public synchronized static String sqlVar(String var) {
	...
	public synchronized String toTpTpForChecker() {
		if (var != null) {
			...
			return sym().toString() + "("
					+ Util.sep(args.stream().map(x -> x.toTpTpForChecker()).collect(Collectors.toList()), ",") + ")";
		} else if (att() != null) {
	...
	public String toString(Function<Sk, String> sk_printer, Function<Gen, String> gen_printer, boolean quote) {
		if (var != null) {
			return var.toString();
		} else if (sym() != null) {
			if (args == null || args.isEmpty()) {
	...
	public String toString(boolean b) {
		return toString(x -> x.toString(), x -> x.toString(), b);
	}
	...
	public static <Ty, En, Sym, Fk, Att, Gen, Sk> Term<Ty, En, Sym, Fk, Att, Gen, Sk> Var(String var) {
	...
	public synchronized Term<Ty, En, Sym, Fk, Att, Gen, Sk> replaceHead(
			Map<Head<Ty, En, Sym, Fk, Att, Gen, Sk>, Term<Ty, En, Sym, Fk, Att, Gen, Sk>> replacee, List<String> vars) {
		if (var != null) {
			return this;
		}
		...
		if (ret != null) {
			...
			Map<String, Term<Ty, En, Sym, Fk, Att, Gen, Sk>> map = new THashMap<>(vars.size());
			int i = 0;
			for (String var : vars) {
	...
	public synchronized Term<Ty, En, Sym, Fk, Att, Gen, Sk> replaceHead(Head<Ty, En, Sym, Fk, Att, Gen, Sk> replacee,
			List<String> vars, Term<Ty, En, Sym, Fk, Att, Gen, Sk> replacer) {
		if (var != null) {
			return this;
		}
	...
	public synchronized Term<Ty, En, Sym, Fk, Att, Gen, Sk> subst(
			Map<String, Term<Ty, En, Sym, Fk, Att, Gen, Sk>> map) {
	...
	public KBExp<Head<Ty, En, Sym, Fk, Att, Gen, Sk>, String> toKB() {
		return this;
	}

	public static <Ty, En, Sym, Fk, Att, Gen, Sk> Term<Ty, En, Sym, Fk, Att, Gen, Sk> fromKB(
			KBExp<Head<Ty, En, Sym, Fk, Att, Gen, Sk>, String> e) {
	...
	@Override
	public synchronized Set<String> getVars() {
		if (vars != null) {
			return vars;
		}
		vars = new THashSet<>();
	...
	@Override
	public List<KBExp<Head<Ty, En, Sym, Fk, Att, Gen, Sk>, String>> getArgs() {
		return (List<KBExp<Head<Ty, En, Sym, Fk, Att, Gen, Sk>, String>>) (Object) args();
	}

	@Override
	public KBExp<Head<Ty, En, Sym, Fk, Att, Gen, Sk>, String> substitute(Map sigma) {
		return subst(sigma);
	}

	@Override
	public KBExp<Head<Ty, En, Sym, Fk, Att, Gen, Sk>, String> replace(List<Integer> l,
			KBExp<Head<Ty, En, Sym, Fk, Att, Gen, Sk>, String> r) {
		if (isVar()) {
			if (l.isEmpty()) {
				return r;
			}
		...
		Iterator<KBExp<Head<Ty, En, Sym, Fk, Att, Gen, Sk>, String>> it = getArgs().iterator();
		int i = 0;
		while (it.hasNext()) {
			KBExp a = it.next();
			if (i == x) {
	...
	public String toCoq(Function<String, String> fn, Set<String> sb, Function<Att, En> fk1, Function<Fk, En> fk2, String pre) {
		if (var != null) {
			return fn.apply(var); 
		} else if (sym() != null) {
			return fn.apply(sym().toString()) + ((args.size()==0) ? "" : (
				"("
					+ Util.sep(args.stream().map(x -> x.toCoq(fn, sb, fk1, fk2, pre)).collect(Collectors.toList()), ", ") 
				+")"));
		} else if (att() != null) {
			if (fk1 != null)
				return pre + fn.apply(fk1.apply(att()).toString()) + "_" + fn.apply(att().toString()) + "(" + arg.toCoq(fn, sb, fk1, fk2, pre) + ")";
			return pre + fn.apply(att().toString()) + "(" + arg.toCoq(fn, sb, fk1, fk2, pre) + ")";
			
		} else if (fk() != null) {
			if (fk2 != null)
				return pre + fn.apply(fk2.apply(fk()).toString()) + "_" + fn.apply(fk().toString()) + "(" + arg.toCoq(fn, sb, fk1, fk2, pre) + ")";
			return pre + fn.apply(fk().toString()) + "(" + arg.toCoq(fn, sb, fk1, fk2, pre) + ")";

		} else if (gen() != null) {
			return fn.apply(gen().toString());
		} else if (sk() != null) {
			return fn.apply(sk().toString());
		} else if (obj() != null) {
			sb.add("Parameter " + fn.apply(obj().toString()) + " : " + fn.apply(ty().toString()) + ".");
			return fn.apply(obj().toString());
		}
		return ("Anomaly: please report");
