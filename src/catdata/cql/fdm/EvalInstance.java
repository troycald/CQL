...
public class EvalInstance<Ty, En1, Sym, Fk1, Att1, Gen, Sk, En2, Fk2, Att2, X, Y> extends
		...
		implements
		DP<Ty, En2, Sym, Fk2, Att2, Row<En2, Chc<X, Term<Ty, En1, Sym, Fk1, Att1, Gen, Sk>>, Chc<En1, Ty>>, Y> {
	...
	public EvalInstance(Query<Ty, En1, Sym, Fk1, Att1, En2, Fk2, Att2> q,
		...
		J = new SaturatedInstance<>(alg, dp(), I.requireConsistency(),
				(boolean) options.getOrDefault(AqlOption.allow_java_eqs_unsafe), false, null);
...
//			checkSatisfaction();
	...
	public boolean eq(Map<String, Chc<Ty, En2>> ctx,
		...
		if (atType(lhs)) {
			return I.dp().eq(null, I.reprT(alg.intoY(lhs)), I.reprT(alg.intoY(rhs)));
		} else {
			return alg.intoX(lhs).equals(alg.intoX(rhs));
		}
