...
public final class QueryExpFront extends QueryExp {
	...
	public QueryExpFront(EdsExp eds, String i, boolean isBack, List<Pair<String, String>> ops) {
		...
		if (options.containsKey(AqlOption.simple_query_entity.name())) {
			throw new RuntimeException("option reserved on front keyword: " + AqlOption.simple_query_entity);
		}
	...
	public Query<String, String, Sym, Fk, Att, String, Fk, Att> eval0(AqlEnv env, boolean isC) {
		...
		var xxx = new QueryExpRawSimple(eds.type(env.typing), 0,
				new PreBlock(gens, eqs, atts, Collections.emptyList(), Util.toList(options), false));
	...
	static void f(Schema<String, String, Sym, Fk, Att> sch, List<Pair<LocStr, String>> gens,
	...
	public static RawTerm conv(Term<String, String, Sym, Fk, Att, Void, Void> t) {
		if (t.var != null) {
