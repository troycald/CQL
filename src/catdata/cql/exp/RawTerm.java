...
//   
	...
	private RawTerm(String head, List<RawTerm> args, String annotation) {
		...
		if (head.contains(("0@Float"))) {
			new RuntimeException("ExA").printStackTrace();
			throw new RuntimeException("ExA");
		}
		if (toString().contains(("\"0@Float\""))) {
			System.out.println(toString());
			new RuntimeException("ExB").printStackTrace();
			throw new RuntimeException("ExB");
		}
	...
	public RawTerm filter(Set<String> s) {
		if (s.contains(head)) {
			if (args.size() != 1) {
				throw new RuntimeException("Warehousing pattern isomorphism name collision");
			}
			return args.get(0).filter(s);
		}
		return new RawTerm(head, args.stream().map(x -> x.filter(s)).collect(Collectors.toList()));
	}

	public RawTerm inferFrom(String m) {
