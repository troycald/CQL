...
public class ED {
	...
	public <Gen, Sk> String tptpX(
			KBTheory<Chc<String, String>, Head<String, String, Sym, Fk, Att, Gen, Sk>, String> th) {
		StringBuffer sb = new StringBuffer("");
		List<String> w = new LinkedList<>();
		if (!As.isEmpty()) {
			sb.append("(! [");
	...
	public <Gen, Sk> String tptp(String x, int i,
			KBTheory<Chc<String, String>, Head<String, String, Sym, Fk, Att, Gen, Sk>, String> th) {
	...
	public LiteralTransform<String, String, Sym, Fk, Att, String, String, String, String, Integer, Chc<String, Pair<Integer, Att>>, Integer, Chc<String, Pair<Integer, Att>>> asTransform(
		...
		validate(sch);
		LiteralInstance<String, String, Sym, Fk, Att, String, String, Integer, Chc<String, Pair<Integer, Att>>> I = front(
				sch), J = back(sch);
	...
	public synchronized final Query<String, String, Sym, Fk, Att, String, Fk, Att> getQ(
...
//  public final Schema<Ty, En, Sym, Fk, Att> schema;
	...
	private final Term<String, String, Sym, Fk, Att, String, String> freeze(
	...
	public <Gen1, Sk1, X1, Y1, Gen2, Sk2, X2, Y2> ED(AqlOptions options,
		...
		for (;;) {
			Set<Pair<Term<String, String, Sym, Fk, Att, Void, Void>, Term<String, String, Sym, Fk, Att, Void, Void>>> z2 = simplify(
					Es, Ewh);
			if (z2 == null) {
				break;
			}
		...
		for (;;) {
			Set<Pair<Term<String, String, Sym, Fk, Att, Void, Void>, Term<String, String, Sym, Fk, Att, Void, Void>>> z2 = simplify2(
					Es, Ewh);
			if (z2 == null) {
				break;
			}
		...
		validate(h.src().schema());
		asTransform(h.src().schema());
	}

	private static Set<Pair<Term<String, String, Sym, Fk, Att, Void, Void>, Term<String, String, Sym, Fk, Att, Void, Void>>> subst(
			String var, Term<String, String, Sym, Fk, Att, Void, Void> t,
		...
		Set<Pair<Term<String, String, Sym, Fk, Att, Void, Void>, Term<String, String, Sym, Fk, Att, Void, Void>>> ret = new THashSet<>(
				wh.size());
		Map<String, Term<String, String, catdata.cql.exp.Sym, catdata.cql.exp.Fk, catdata.cql.exp.Att, Void, Void>> m = Collections
				.singletonMap(var, t);
		for (Pair<Term<String, String, catdata.cql.exp.Sym, catdata.cql.exp.Fk, catdata.cql.exp.Att, Void, Void>, Term<String, String, catdata.cql.exp.Sym, catdata.cql.exp.Fk, catdata.cql.exp.Att, Void, Void>> eq : wh) {
			Term<String, String, catdata.cql.exp.Sym, catdata.cql.exp.Fk, catdata.cql.exp.Att, Void, Void> a = eq.first
					.subst(m);
			Term<String, String, catdata.cql.exp.Sym, catdata.cql.exp.Fk, catdata.cql.exp.Att, Void, Void> b = eq.second
					.subst(m);
			if (!a.equals(b)) {
	...
	private static Set<Pair<Term<String, String, Sym, Fk, Att, Void, Void>, Term<String, String, Sym, Fk, Att, Void, Void>>> simplify2(
		...
		Iterator<Pair<Term<String, String, Sym, Fk, Att, Void, Void>, Term<String, String, Sym, Fk, Att, Void, Void>>> it = in
				.iterator();
		while (it.hasNext()) {
			Pair<Term<String, String, Sym, Fk, Att, Void, Void>, Term<String, String, Sym, Fk, Att, Void, Void>> eq = it
					.next();
	...
	private Set<Pair<Term<String, String, Sym, Fk, Att, Void, Void>, Term<String, String, Sym, Fk, Att, Void, Void>>> simplify(
		...
		outer: while (it.hasNext()) {
			...
			if (eq.second.var != null && Es.containsKey(eq.second.var)) {
				...
				Set<Pair<Term<String, String, Sym, Fk, Att, Void, Void>, Term<String, String, Sym, Fk, Att, Void, Void>>> set = new THashSet<>();
				for (Pair<Term<String, String, Sym, Fk, Att, Void, Void>, Term<String, String, Sym, Fk, Att, Void, Void>> eq2 : in) {
					Map<String, Term<String, String, Sym, Fk, Att, Void, Void>> m = Collections
							.singletonMap(eq.second.var, eq.first);
					set.add(new Pair<>(eq2.first.subst(m), eq2.second.subst(m)));
				}
				return set;
			}
			if (eq.first.var != null && Es.containsKey(eq.first.var)) {
				...
				for (Pair<Term<String, String, Sym, Fk, Att, Void, Void>, Term<String, String, Sym, Fk, Att, Void, Void>> eq2 : in) {
					Map<String, Term<String, String, Sym, Fk, Att, Void, Void>> m = Collections
							.singletonMap(eq.first.var, eq.second);
					set.add(new Pair<>(eq2.first.subst(m), eq2.second.subst(m)));
				}
				return set;
			}
	...
	public void validate(Schema<String, String, Sym, Fk, Att> sch) {
		for (Pair<Term<String, String, catdata.cql.exp.Sym, catdata.cql.exp.Fk, catdata.cql.exp.Att, Void, Void>, Term<String, String, catdata.cql.exp.Sym, catdata.cql.exp.Fk, catdata.cql.exp.Att, Void, Void>> x : Awh) {
...
//		}
	...
	public ED sigma(Mapping<String, String, Sym, Att, Fk, String, Fk, Att> F) {
		Map<String, Chc<String, String>> As0 = new HashMap<>(As.size());
		Map<String, Chc<String, String>> Es0 = new HashMap<>(Es.size());
		Set<Pair<Term<String, String, Sym, Fk, Att, Void, Void>, Term<String, String, Sym, Fk, Att, Void, Void>>> Awh0 = new HashSet<>(
				Awh.size());
		Set<Pair<Term<String, String, Sym, Fk, Att, Void, Void>, Term<String, String, Sym, Fk, Att, Void, Void>>> Ewh0 = new HashSet<>(
				Ewh.size());

		for (var entry : As.entrySet()) {
			if (!entry.getValue().left) {
				As0.put(entry.getKey(), Chc.inRight(F.ens.get(entry.getValue().r)));
			} else {
				As0.put(entry.getKey(), entry.getValue());
			}
		}
		for (var entry : Es.entrySet()) {
			if (!entry.getValue().left) {
				Es0.put(entry.getKey(), Chc.inRight(F.ens.get(entry.getValue().r)));
			} else {
				Es0.put(entry.getKey(), entry.getValue());
			}
		}
		for (var eq : Awh) {
			Awh0.add(new Pair<>(F.trans(eq.first.convert()), F.trans(eq.second.convert())));
		}
		for (var eq : Ewh) {
			Ewh0.add(new Pair<>(F.trans(eq.first.convert()), F.trans(eq.second.convert())));
		}

		var ret = new ED(As0, Es0, Awh0, Ewh0, isUnique, options);
	...
	public String toSql(Schema<String, String, Sym, Fk, Att> sch) {
		...
		for (var x : Util.alphabetical(Es.entrySet())) {
			...
			List<Pair<Att, String>> lx = new LinkedList<>();
			for (var eq : Util.alphabetical(Ewh)) {
				if (eq.first.att() == null || !eq.first.arg.var.equals(x.getKey()))
					continue;

				if (eq.second.att() != null && !As.keySet().contains(eq.second.arg.var)) {
					lx.add(new Pair<>(eq.first.att(), "NULL" + " AS [" + eq.first.att() + "]"));
				} else {
					lx.add(new Pair<>(eq.first.att(), eq.second + " AS [" + eq.first.att() + "]"));
				}
			}
