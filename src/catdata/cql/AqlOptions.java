...
public final class AqlOptions {
	...
	public enum AqlOption {
		maedmax_path, allow_sql_import_all_unsafe, jena_reasoner, allow_aggregation_unsafe, import_sql_direct_prefix,
		...
		sql_constraints_simple, e_use_auto, is_oracle, check_warn_instead_of_fail, quotient_use_chase, chase_style,
		allow_empty_sorts_unsafe, program_allow_nonconfluence_unsafe, active_domain, gui_sample, gui_sample_size,
		import_dont_check_closure_unsafe, js_env_name, interpret_as_algebra, csv_field_delim_char, csv_escape_char,
		csv_quote_char, csv_file_extension, csv_generate_ids, emit_ids, id_column_name, always_reload, varchar_length,
		gui_max_table_size, gui_max_graph_size, gui_max_string_size, gui_rows_to_display, gui_show_atts, random_seed,
		num_threads, eval_max_temp_size, eval_reorder_joins, eval_max_plan_depth, eval_join_selectivity,
		eval_use_indices, eval_use_sql_above, eval_approx_sql_unsafe, eval_sql_persistent_indices,
		query_remove_redundancy, import_null_on_err_unsafe, tinkerpop_host, tinkerpop_port, tinkerpop_graph_name,
		bitsy_db_path, simplify_names, left_bias, jdbc_quote_char, map_nulls_arbitrarily_unsafe, jdbc_default_class,
	...
	private static Object getDefault(AqlOption option) {
		...
		case jdbc_zero -> "(SELECT '0' AS CQL_ZERO)";
		case active_domain -> false;
		case tinkerpop_host -> "localhost";
		case tinkerpop_port -> 8182;
		case tinkerpop_graph_name -> "g";
		case bitsy_db_path -> ".";
		case allow_sql_import_all_unsafe -> true;
		case allow_aggregation_unsafe -> false;
		case csv_entity_name -> "E";
		case import_sql_direct_prefix -> "";
		case left_bias -> false;
		case sql_constraints_simple -> true;
		case static_timeout -> 10L;
		...
		case allow_java_eqs_unsafe -> true;
		...
		case e_use_auto -> true;
		case is_oracle -> false;
//	default -> throw new IllegalArgumentException("Unexpected value: " + option);
	...
	private static Object getFromMap(Map<String, String> map, /* Collage<Ty, En, Sym, Fk, Att, Gen, Sk> col, */
		...
		case jdbc_zero -> op.getString(map);
		case tinkerpop_host -> op.getString(map);
		case tinkerpop_port -> op.getInteger(map);
		case tinkerpop_graph_name -> op.getString(map);
		case bitsy_db_path -> op.getString(map);
		case active_domain -> op.getBoolean(map);
		...
		case e_use_auto -> op.getBoolean(map);
		case is_oracle -> op.getBoolean(map);
