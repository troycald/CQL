...
public class AqlProver<Ty, En, Sym, Fk, Att, Gen, Sk> implements DP<Ty, En, Sym, Fk, Att, Gen, Sk> {
	...
	private AqlProver(AqlOptions ops, Collage<Ty, En, Sym, Fk, Att, Gen, Sk> col, AqlJs<Ty, Sym> js,
			boolean doTrivialityCheck) {
		...
		DPKB<Chc<Ty, En>, Head<Ty, En, Sym, Fk, Att, Gen, Sk>, String> dpkb;
		switch (name) {
		case auto:
		...
		case completion:
			...
			if (str != null) {
				...
				Collage<String, String, catdata.cql.exp.Sym, catdata.cql.exp.Fk, catdata.cql.exp.Att, String, String> col2 = (Collage<String, String, catdata.cql.exp.Sym, catdata.cql.exp.Fk, catdata.cql.exp.Att, String, String>) col;
				for (String x : z) {
					try {
						Head o = catdata.cql.exp.RawTerm.toHeadNoPrim(x, col2);
						prec.add(o);
					} catch (Exception ex) {
					}
				}
			...
			break;
		case monoidal:
			dpkb = new catdata.cql.MonoidalFreeDP<>(col_simpl.toKB());
			break;
		case e:
			String exePath = (String) ops.getOrDefault(AqlOption.e_path);
			boolean auto = (boolean) ops.getOrDefault(AqlOption.e_use_auto);
			dpkb = new EProver<>(exePath, col_simpl.toKB(), timeout, auto);
			break;
		case vampire:
		...
		if (doTrivialityCheck && (boolean) ops.getOrDefault(AqlOption.triviality_check_best_effort)
				&& dpkb.supportsTrivialityCheck()) {
			List<Chc<Ty, En>> trivial = Util.list();
			try {
				String x = (Util.uniqueName()), y = (Util.uniqueName());
				KBExp<Head<Ty, En, Sym, Fk, Att, Gen, Sk>, String> l = dpkb.kb.factory.KBVar(x),
						r = dpkb.kb.factory.KBVar(y);
				for (Chc<Ty, En> sort : dpkb.kb.inhabGen()) {
					Map<String, Chc<Ty, En>> m = Util.mk();
					m.put(x, sort);
					m.put(y, sort);
					if (dpkb.eq(m, l, r)) {
						trivial.add(sort);
