...
public class SqlTypeSide2<Ty> {
	...
	public synchronized TypeSide<Ty, Sym> make(TypeSide<Ty, Sym> parent, AqlOptions ops) {
		...
		System.out.println(tys(parent));
		return new TypeSide<Ty, Sym>(tys(parent), syms(parent), eqs(parent), Collections.emptyMap(),
				Collections.emptyMap(), Collections.emptyMap(), ops);
	}
	...
	public Term<Ty, Void, Sym, Void, Void, Void, Void> t = Term.Obj(Optional.of(true), "boolean").convert();
	public Term<Ty, Void, Sym, Void, Void, Void, Void> f = Term.Obj(Optional.of(false), "boolean").convert();
	...
	private Set<Triple<Map<String, Ty>, Term<Ty, Void, Sym, Void, Void, Void, Void>, Term<Ty, Void, Sym, Void, Void, Void, Void>>> eqs(
		...
		for (Ty ty : tys(parent)) {
			ret.add(new Triple<>(Collections.emptyMap(), Term.Sym(typedIsNull(ty),
					Collections.singletonList(Term.Sym(typedNull(ty), Collections.emptyList()))), t));
		}
