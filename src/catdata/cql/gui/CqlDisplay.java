...
public final class CqlDisplay implements Disp {
...
//    JComponent comp = AqlViewer.view(obj, max_rows, exp, env, showAtts);
		LazyPanel p = new LazyPanel((u) -> CqlViewer.view(c, obj, max_rows, exp, env, showAtts));
		return p;
	}

	public CqlDisplay(String title, Program<Exp<?>> p, AqlEnv env, long start, long middle) {
		this.env = env;
		this.title = title;
	...
	private static Exp<?> unresolve(Program<Exp<?>> p, Exp<?> e) {
